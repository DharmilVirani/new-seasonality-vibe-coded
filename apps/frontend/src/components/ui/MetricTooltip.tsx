...
const defaultMetrics: Record<string, { label: string; formula: string }> = {
  avgReturn: { label: 'Average Return', formula: 'Σ(Returns) ÷ Count' },
  sumReturn: { label: 'Total Return', formula: 'Σ(Returns) or Product(1+r)-1' },
  cagr: { label: 'CAGR', formula: '(End/Start)^(1/years) - 1' },
  maxDrawdown: { label: 'Max Drawdown', formula: '(Trough - Peak) ÷ Peak' },
  winRate: { label: 'Win Rate', formula: '(Wins ÷ Total) × 100' },
  lossRate: { label: 'Loss Rate', formula: '(Losses ÷ Total) × 100' },
  stdDev: { label: 'Std Dev', formula: '√Variance(Returns)' },
  volatility: { label: 'Volatility', formula: 'σ × √Periods' },
  sharpeRatio: { label: 'Sharpe Ratio', formula: '(Return - RiskFreeRate) ÷ σ' },
  sortinoRatio: { label: 'Sortino Ratio', formula: '(Return - RiskFreeRate) ÷ σ_down' },
  totalCount: { label: 'Total Periods', formula: 'Count of periods' },
  positiveCount: { label: 'Winning Periods', formula: 'Count of periods with Return > 0' },
  negativeCount: { label: 'Losing Periods', formula: 'Count of periods with Return < 0' },
  eventCount: { label: 'Total Events', formula: 'Occurrence count' },
  weekNumber: { label: 'Week Number', formula: 'ISO week (1-52)' },
  monthNumber: { label: 'Month Number', formula: 'Calendar month (1-12)' },
  year: { label: 'Year', formula: 'Calendar year' },
  returnPercentage: { label: 'Return %', formula: '((Close - Open) ÷ Open) × 100' },
  median: { label: 'Median Return', formula: 'Middle value of sorted returns' },
  consecutiveWins: { label: 'Win Streak', formula: 'Max consecutive wins' },
  consecutiveLosses: { label: 'Loss Streak', formula: 'Max consecutive losses' },
};
